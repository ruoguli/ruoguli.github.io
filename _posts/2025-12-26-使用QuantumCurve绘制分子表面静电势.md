...
## 2. 绘制分子表面静电势
 ...
 载入文件后，选择Analysis中的Electrostatic potential (ESP)即可。其余部分如无必要，不用修改。
 点击Run，需要等待一段时间才能看到结果⌛️。现在计算的效率还不够高，还需要优化。
 计算完成后关闭Electrostatic potential (ESP)界面。输入文件的路径中会出现一个benzene_ESP.cube文件。
